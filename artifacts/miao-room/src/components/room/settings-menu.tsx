...
function SkipLoginAlert({ open, onConfirm, onCancel }: { open: boolean; onConfirm: () => void; onCancel: () => void }) {
  if (!open) return null
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-foreground/30 backdrop-blur-sm animate-in fade-in">
      <div className="mx-4 w-full max-w-sm rounded-3xl border border-border bg-card p-5 shadow-2xl animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-3 text-sm text-muted-foreground">跳过登录可能导致本地数据丢失，包括：</p>
        <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
          {['角色进度和数据', '收藏的信件和相册', '仓库中的物品'].map((item) => (
            <li key={item} className="flex items-center gap-2">
              <span className="size-1.5 rounded-full bg-destructive" />{item}
            </li>
          ))}
        </ul>
        <div className="mt-4 flex gap-2">
          <button onClick={onCancel} className="flex-1 rounded-xl border border-border py-2 font-cute text-sm text-muted-foreground transition hover:bg-secondary">
            返回登录
          </button>
          <button onClick={onConfirm} className="flex-1 rounded-xl bg-destructive py-2 font-cute text-sm text-destructive-foreground transition hover:brightness-110">
            确定跳过
          </button>
        </div>
      </div>
...
function LoginForm({ method, onBack, onSuccess }: { method: 'phone' | 'email'; onBack: () => void; onSuccess: () => void }) {
...
export function SettingsMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [loginStep, setLoginStep] = useState<'menu' | 'method' | 'phone' | 'email'>('menu')
  const [showSkipAlert, setShowSkipAlert] = useState(false)
  const [isLoggedIn, setIsLoggedIn] = useState(false)
  const [isGuest, setIsGuest] = useState(false) // 跳过登录后的游客状态
        ...
        <div className="mt-2 border-t-2 border-border/50 pt-2">
          {isLoggedIn && userInfo ? (
            ...
            <div className="space-y-1">
              ...
              >
                <User className="size-4 text-primary" />
                <span className="font-cute text-sm text-foreground">去登录</span>
              </button>
            </div>
          ) : (
            // 未登录
            <div className="space-y-2">
              {loginStep === 'menu' ? (
                ...
                <div className="px-3 py-2 space-y-2">
                  <button onClick={() => setLoginStep('menu')} className="text-xs text-muted-foreground hover:text-foreground">
                    ← 返回
                  </button>
                ...
                <div className="px-3 py-2">
                  <LoginForm method={loginStep} onBack={() => setLoginStep('method')} onSuccess={handleLoginSuccess} />
                </div>
              ) : null}
            </div>
          )}
